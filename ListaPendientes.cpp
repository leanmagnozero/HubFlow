...
void ListaPendientes::insertar(Envio* envio) {
    ...
    // Caso 2: el nuevo tiene MAYOR prioridad que la cabeza actual
    // -> va al inicio. Ojo con el uso de tieneMayorPrioridad y no
    // "<=": así, a igual prioridad, el nuevo NO desplaza al que ya
    // estaba primero (eso es lo que garantiza la estabilidad FIFO).
    if (tieneMayorPrioridad(envio->getServicio(), cabeza_->envio->getServicio())) {
    ...
    while (anterior->siguiente != nullptr &&
           !tieneMayorPrioridad(envio->getServicio(), anterior->siguiente->envio->getServicio())) {
